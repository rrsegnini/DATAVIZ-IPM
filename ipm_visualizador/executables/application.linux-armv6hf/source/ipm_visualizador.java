...
public class ipm_visualizador extends PApplet {
...
class Indicator {
  ...
  int[] numbers = {0, 0, 0, 0};
  String[] indicators = {"", "", "", ""};
  int[] colors = {0xff0054a6, 0xffed145b, 0xffffb000, 0xff7cc576};
...
public void setup() {
  ...
  r = cp5.addRadioButton("dimensionRadio")
         ...
         .setColorBackground(color(0xff3C55D7))
...
public void dimensionRadio(int a) {
  switch(a) {
...
public void map(int year, String dimension, Region region) {
  ...
  // Map size
  float m = map(dimTotal, 0, max, 0, 1.5f);
  // Map color
  ...
  region.R = PApplet.parseInt(r);
  region.G = PApplet.parseInt(g);
  
}

// Finds the max value for a certain year and dimension
public int findMax(int year, String dimension) {
  int[] max = {0, 0, 0, 0, 0, 0};
  for (TableRow row : data.rows()) {
    if (year == row.getInt("Año") && dimension.equals(row.getString("Dimensión"))) {
...
public void drawTreemap(int year, String dimension, Region region) {
  PGraphics t;
...
public void drawRect(Indicator t, int x1, int y1, int w1, int h1, int value) {
  t.treemap.imageMode(CENTER);
  t.treemap.beginDraw();
  t.treemap.colorMode(HSB, 1.0f);
  t.treemap.stroke(1);
...
public int getPerfectSplitNumber(int[] numbers, int blockW, int blockH) {
  ...
  float ratio = PApplet.parseFloat(valueA) / PApplet.parseFloat(valueB + valueA);
...
public void makeBlock(Indicator t, int refX, int refY, int blockW, int blockH, int[] numbers) {
  ...
  public void settings() {  size(1300, 1000, P2D);  smooth(); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "ipm_visualizador" };
    if (passedArgs != null) {
